local CosineCriterion, parent = torch.class('nn.CosineCriterion', 'nn.Criterion')

function CosineCriterion:__init(margin)
   parent.__init(self)
   margin=margin or 0
   self.margin = margin 
...
function CosineCriterion:updateOutput(input,y)
...
function CosineCriterion:updateGradInput(input, y)
   local v1  = input[1]
   local v2  = input[2]
   ...
   self.gradInput = torch.cat(gw1, gw2, 1)
